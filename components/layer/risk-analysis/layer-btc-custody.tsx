...
import BitcoinScriptAnalysisDropdown from "@/components/bitcoin-script-analysis-dropdown";
...
const BtcCustody: React.FC<BtcCustodyProps> = ({ category, pegs = [], layerName }) => {
    ...
    // Fetch Bitcoin script analysis for this layer
    const { data: bitcoinAnalysis, isLoading: isBitcoinAnalysisLoading } = useQuery<BitcoinTransactionAnalysis>({
        queryKey: ['bitcoin-analysis', layerName],
        queryFn: () => layerName ? fetcher(`/api/bitcoin-analysis/${layerName}`) : null,
        enabled: !!layerName,
    ...
    return (
        <div className="flex flex-col justify-start items-start gap-2">
            ...
            {layerName && (
                <div className="mt-6">
                    <BitcoinScriptAnalysisDropdown 
                        analysis={bitcoinAnalysis || null}
                        layerName={layerName}
                    />
                </div>
            )}

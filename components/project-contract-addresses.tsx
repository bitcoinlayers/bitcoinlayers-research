...
import GovernanceAnalysisDropdown from "./governance-analysis-dropdown";
import { AnalysisReport } from "@/util/load-analysis-report";
...
const AddressItem = ({
    ...
    useEffect(() => {
        const loadAnalysis = async () => {
            if (!item.token_address) return;
            
            setIsLoadingAnalysis(true);
            try {
                // Fetch analysis report from API endpoint
                // If this is for a layer, pass the layer slug as a parameter
                const layerParam = isLayer ? `?layer=${item.network_slug}` : '';
                const response = await fetch(`/api/analysis/${item.token_address}${layerParam}`);
                if (response.ok) {
